...
unsigned long g_high_prio_arg = 0;
void high_prio_fn(grpc_channel_stack *channel_stack, grpc_channel_element *elem,
                  void *arg) {
  unsigned long *i = arg;
  GPR_ASSERT(*i == 0);
  *i = 0xdeadbeef;
...
bool g_low_prio_fn_called = false;
void low_prio_fn(grpc_channel_stack *channel_stack, grpc_channel_element *elem,
                 void *arg) {
  g_low_prio_fn_called = true;
}
...
static void test_channel_stack_builder_filter_replace(void) {
  ...
  GPR_ASSERT(g_high_prio_arg == 0xdeadbeef);
  // ... and that the low priority one hasn't.
  GPR_ASSERT(!g_low_prio_fn_called);
  grpc_channel_destroy(channel);
}

const grpc_channel_filter filter_high_prio = {
    call_func,
    channel_func,
    0,
    call_init_func,
...
const grpc_channel_filter filter_low_prio = {
    call_func,
    channel_func,
    0,
    call_init_func,
...
static bool add_high_prio_filter(grpc_exec_ctx *exec_ctx,
  ...
  return grpc_channel_stack_builder_prepend_filter(
      builder, filter, high_prio_fn, &g_high_prio_arg);
}

static bool add_low_prio_filter(grpc_exec_ctx *exec_ctx,
                                grpc_channel_stack_builder *builder,
                                void *arg) {
  return grpc_channel_stack_builder_prepend_filter(
      builder, (const grpc_channel_filter *)arg, low_prio_fn, NULL);
}

static void init_plugin(void) {
  grpc_channel_init_register_stage(GRPC_CLIENT_CHANNEL, INT_MAX,
                                   add_low_prio_filter,
                                   (void *)&filter_low_prio);
  grpc_channel_init_register_stage(GRPC_CLIENT_CHANNEL, INT_MAX,
                                   add_high_prio_filter,
                                   (void *)&filter_high_prio);
}
